...
export const PDFTemplate: React.FC<PDFTemplateProps> = ({ quote, business }) => {
  return (
    <div id="pdf-render-target" className="p-8 bg-white text-slate-900" style={{ width: '800px', minHeight: '1120px' }}>
      {/* Header */}
      <div className="flex justify-between items-start border-b-2 border-orange-500 pb-6 mb-6">
        <div className="flex gap-4 items-center">
          ...
          <div>
            <h1 className="text-3xl font-black text-slate-800 uppercase tracking-tight">{business.companyName || 'Serralheria Pro'}</h1>
            <p className="text-slate-500 text-sm font-medium">{business.ownerName}</p>
            <p className="text-slate-500 text-sm">{business.phone} | {business.email}</p>
      ...
      {/* Items Table - Prices removed from PDF view as requested */}
          ...
          {quote.items.map((item) => (
            <tr key={item.id} className="text-sm">
              <td className="p-3">
                ...
                <div className="text-xs text-slate-500 font-medium">{item.material} - {item.description}</div>
              </td>
              <td className="p-3 text-slate-600">{item.width}m x {item.height}m</td>
      ...
      <div className="grid grid-cols-3 gap-8">
        <div className="col-span-2 space-y-4">
          <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest border-b pb-1">Fotos do Projeto</h3>
        ...
        <div className="bg-slate-900 text-white p-6 rounded-2xl shadow-xl h-fit">
          <div className="space-y-3">
            <div className="flex justify-between items-end">
              ...
              <span className="text-2xl font-black">R$ {quote.total.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="mt-12 pt-8 border-t border-slate-100 text-center">
        ...
        <p className="text-[10px] text-slate-300 mt-1">Este documento é um registro de orçamento para fins informativos.</p>
      </div>
    </div>
  );
};
